(()=>{
  ...
  function renderCases(){
    ...
    list.querySelectorAll('[data-case]').forEach(b=>b.addEventListener('click',()=>{current=Number(b.dataset.case);resetCase();renderCase()}));
    $('#caseGenerator').addEventListener('click',()=>{$('#generatorPanel').hidden=false});
  }

  function renderScene(){
    const c=cases[current],moment=c.timeline[sceneIndex];
    ...
    $('#recapCount').textContent=`${sceneIndex+1} moment${sceneIndex===0?'':'s'}`;
    $('#timeline').innerHTML=c.timeline.slice(0,sceneIndex+1).map(t=>`<article class="time-card"><b>${t[0]}</b><p>${t[1]}</p></article>`).join('');
    const btn=$('#nextMoment');
    if(sceneIndex<c.timeline.length-1){btn.textContent='WHAT HAPPENED NEXT? →';btn.classList.remove('done')}else if(!sceneComplete){btn.textContent='START DOCUMENTING →';btn.classList.add('done')}else{btn.textContent='SCENE COMPLETE ✓';btn.disabled=true}
  }

  function nextScene(){
    const c=cases[current];
    if(sceneIndex<c.timeline.length-1){sceneIndex++;renderScene();return}
    if(!sceneComplete){sceneComplete=true;$('#workspace').hidden=false;$('#nextMoment').textContent='SCENE COMPLETE ✓';$('#nextMoment').disabled=true;renderEvidence();renderSlots();updateCounts();$('#workspace').scrollIntoView({behavior:'smooth',block:'start'})}
  }
  ...
  function init(){
    ...
    $('#topicBubbles').querySelectorAll('button').forEach((b,i)=>b.addEventListener('click',()=>{const map=[0,1,2,3,4,5,6,7,Math.floor(Math.random()*cases.length)];current=map[i];resetCase();close();renderCase()}));
  }
  init();
})();